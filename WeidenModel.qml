...
//                console.log(weidenModel.directions.indexOf(data[i][2]),Math.floor((data[i][3]/5)))
                ...
                var s = data[i][3];
                var sIndex = Math.floor((s/5))*5;
                var g = data[i][4];
                var gIndex = Math.floor((g/5))*5;
                var d = data[i][2];
...
//        request("weatherstat_kn.html");
        request("http://www.yachtfinder.info/wx/wx001/weatherstat_kn.html");
    }
